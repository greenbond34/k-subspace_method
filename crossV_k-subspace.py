...
def clustering_by_subspace(test_data, subspaces, k, dim) -> np.ndarray:
    ...
    for v, data in enumerate(test_data):
        for i in range(len(data)):
            for j in range(k):
                norm_list[j] = sum(
                    np.dot(data[i, :], subspaces[j, :, v]) ** 2 for v in range(dim)
                )
...
def calculate_norm(data_test, subspace, dim):
    ...
    for k in range(len(subspace)):
        for i in range(len(data_test)):
            ...
            norm = sum(
                np.dot(data_test[i, :], subspace[k, :, v]) ** 2 for v in range(dim)
            )
...
def make_allclass_subspaces(X_train, y_train, k=5, dim=10, threshold=0.90):
    ...
    if MULTIPROCESS:
        ...
        # 非並列処理
        all_class_subspaces = [
            make_subspaces_for_a_number(bar[i], i, k, dim, threshold)
...
def make_subspaces_for_a_number(X_train, y, k, dim, threshold, clustering="kmeans"):
    ...
    while flag:
        ...
        for i, data in enumerate(train_data):
            subspaces[i] = make_subspace(
                data, dim=dim
            )  # k組目のデータから部分空間を作成
        pred_list, counter = clustering_by_subspace(train_data, subspaces, k, dim)
        # While終了判定
        if any(
            [i > threshold for i in counter]
        ):  # 元のデータによって作られた空間にそのデータが割り振られた割合が90%を超えたら終了
            flag = False
        elif any(
            [len(v) == 0 for v in pred_list]
        ):  # ある集合のデータ数が0になっても終了
            flag = False
        else:
...
def main():
        ...
        for dim in range(1, max_dim + 1):
            ...
            for divi in range(5):
                ...
                X_train, X_test, y_train, y_test = train_test_split(
                    data, target, test_size=0.16, random_state=divi
                )
                all_class_subspaces = make_allclass_subspaces(
                    X_train, y_train, k, dim, threshold
                )
    ...
    for k_num in range(len(store_rec_rate)):
        plt.plot(
            store_dimension[k_num],
            store_rec_rate[k_num],
            linestyle=line_formats[k_num],
            label=f"k={k_array[k_num]}",
        )
    plt.xlim(
        10,
    )
